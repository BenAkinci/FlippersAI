...
let rows=[],sessionId='',refreshing=false,timer=0
...
function card(c,key){const a=analysis(c),url=c.source_url||'',title=c.title||a.identified_name||'Untitled listing',canCompare=key==='shortlist'&&rated(c);return `<article class="v088-listing-card" data-v088-id="${esc(c.id)}">
  ...
  <p>${esc(reason(c))}</p>
  <div class="v088-card-actions"><button data-v088-act="save" ${c.saved?'disabled':''}>${c.saved?'Saved ✓':'♡ Save'}</button><button class="primary" data-v088-act="analyse">Analyse</button><button data-v088-act="edit">Edit info</button>${canCompare?'<button data-v088-act="compare">Compare</button>':''}</div>
</article>`}
function bucketDef(key,label){const list=bucketRows(key),open=openBuckets.has(key);return {key,label,list,open}}
function markup(){
  const defs=[bucketDef('found','FOUND'),bucketDef('rated','RATED'),bucketDef('working','WORKING'),bucketDef('shortlist','SHORTLIST'),bucketDef('filtered','FILTERED OUT')]
  return `<section id="v088ScoutBuckets" class="v088-buckets"><div class="v088-bucket-grid">${defs.map(x=>`<button class="v088-bucket-button ${x.key==='shortlist'?'shortlist':''} ${x.open?'open':''}" data-v088-bucket="${x.key}" aria-expanded="${x.open}"><span>${x.label}</span><strong>${x.list.length}</strong><i>${x.open?'⌃':'⌄'}</i></button>`).join('')}</div><div class="v088-open-panels">${defs.filter(x=>x.open).map(x=>`<section class="v088-panel" data-v088-panel="${x.key}"><header><div><span>${x.label}</span><strong>${x.list.length} listing${x.list.length===1?'':'s'}</strong></div><button data-v088-close="${x.key}" aria-label="Close ${x.label}">×</button></header>${x.list.length?`<div class="v088-panel-list">${x.list.map(c=>card(c,x.key)).join('')}</div>`:`<div class="v088-empty">No listings are currently in this stage.</div>`}</section>`).join('')}</div></section>`
}
...
function bind(box){
  $$('[data-v088-bucket]',box).forEach(b=>b.onclick=()=>{const key=b.dataset.v088Bucket;openBuckets.has(key)?openBuckets.delete(key):openBuckets.add(key);render()})
  $$('[data-v088-close]',box).forEach(b=>b.onclick=()=>{openBuckets.delete(b.dataset.v088Close);render()})
...
function render(){const summary=$('.scout-summary'),oldList=$('.scout-list');if(!summary)return;summary.classList.add('v088-replaced');oldList?.classList.add('v088-replaced-list');const box=host();if(!box)return;box.outerHTML=markup();const next=$('#v088ScoutBuckets');if(next)bind(next)}
async function refresh(force=false){if(refreshing)return;refreshing=true;try{if(await load())render()}finally{refreshing=false}}
function schedule(){clearTimeout(timer);timer=setTimeout(()=>refresh().catch(()=>{}),80)}

document.addEventListener('flippers:scout-rendered',schedule)
document.addEventListener('flippers:candidate-updated',schedule)
document.addEventListener('click',e=>{if(e.target.closest?.('.ext-nav [data-view="scan"]'))setTimeout(schedule,80)},true)
new MutationObserver(ms=>{if(ms.some(m=>m.addedNodes.length)&&$('.scout-page-head[data-scout-session]'))schedule()}).observe(document.getElementById('app'),{childList:true,subtree:true})
schedule()
