...
function renderScout(){
  ...
  const detected=Number(scout.session?.metadata?.total_detected||scout.candidates.length),round=roundRows().length
...
async function startOver(){scout=null;await chrome.storage.local.remove(ACTIVE_SCOUT_KEY);location.reload()}
function bindUi(){
...
async function createSession(collection){
  ...
  const all=collection.candidates||[],candidates=all.slice(0,MAX_FOUND_PER_ROUND)
  const metadata={source:'chrome_extension',collection_scan:true,captured_at:collection.capturedAt||new Date().toISOString(),found_limit:MAX_FOUND_PER_ROUND,screen_batch_max:10,total_detected:all.length,current_round:1}
  const session=await api.insert('scout_sessions',{user_id:user.id,platform:collection.platform||'other',source_url:collection.pageUrl||'',query_text:genericQuery(collection.query)?'':(collection.query||''),status:'draft',candidate_count:candidates.length,selected_count:0,metadata},{single:true})
  if(!session?.id)throw new Error('Could not create the Scout Session.')
  const rows=candidates.map((c,i)=>({session_id:session.id,user_id:user.id,source_url:c.url,listing_id:c.listingId||null,title:c.title||null,asking_price:c.askingPrice??null,currency:c.currency||'AUD',location:c.location||null,condition:c.condition||null,seller_name:c.sellerName||null,thumbnail_url:c.thumbnailUrl||null,region_code:c.regionCode||null,category_label:c.categoryLabel||'Other',raw_capture:{raw_text:c.rawText||'',quick_scan:true,order_index:i,round_index:1,region_code:c.regionCode||'',category_label:c.categoryLabel||'Other'},scan_status:'quick',selected:false,rank_score:null,saved:false}))
  const saved=rows.length?await api.insert('scout_candidates',rows):[]
  return {session:{...session,metadata},candidates:(saved||[]).map((row,i)=>({...row,order_index:i,analysis:row.analysis||{}}))}
}
async function startCollection(collection){const created=await createSession(collection);scout={session:created.session,candidates:created.candidates,tabId:collection.tabId,platform:collection.platform,query:collection.query||'',pageUrl:collection.pageUrl||''};await persistScout();renderScout()}
async function mergeNew(collection){
  const known=new Set(scout.candidates.map(c=>c.source_url)),all=(collection.candidates||[]).filter(c=>c.url&&!known.has(c.url)),fresh=all.slice(0,MAX_FOUND_PER_ROUND);if(!fresh.length)return 0
  const user=await api.getUser(),start=scout.candidates.length,round=currentRound()+1
  const rows=fresh.map((c,i)=>({session_id:scout.session.id,user_id:user.id,source_url:c.url,listing_id:c.listingId||null,title:c.title||null,asking_price:c.askingPrice??null,currency:c.currency||'AUD',location:c.location||null,condition:c.condition||null,seller_name:c.sellerName||null,thumbnail_url:c.thumbnailUrl||null,region_code:c.regionCode||null,category_label:c.categoryLabel||'Other',raw_capture:{raw_text:c.rawText||'',quick_scan:true,order_index:start+i,round_index:round,region_code:c.regionCode||'',category_label:c.categoryLabel||'Other'},scan_status:'quick',selected:false,rank_score:null,saved:false}))
  const saved=await api.insert('scout_candidates',rows);scout.candidates.push(...(saved||[]).map((r,i)=>({...r,order_index:start+i,analysis:r.analysis||{}})))
