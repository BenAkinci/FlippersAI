...
expect(maj>0||min>89||(min===89&&patch>=4),'manifest must package v0.89.4 or newer')
expect(!smart.includes("lastHtml='',lastHtml=''"),'lastHtml must never be declared twice')
expect((orch.match(/const economicLoss=c=>/g)||[]).length<=1,'legacy economicLoss declaration must not duplicate')
expect(!/(?<!\$)\$\('\[data-v088-source-link\]',main\)\.forEach/.test(work),'source-link iteration must use $$, not $')
...
console.log('v0.89.4 packaged-extension integrity protections preserved')
