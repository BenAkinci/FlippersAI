(() => {
  ...
  function parsePrice(value) {
    ...
    text = text.replace(/AUD|USD|GBP/gi, ' ').replace(/US\$/gi, ' ').replace(/A\$/gi, ' ').replace(/[£$]/g, ' ').replace(/\b(?:PRICE|COST|ASK|ASKING)\b/gi, ' ').trim()
  ...
  function isFootwearContext(form) {
    const fields = ['title','brand','model','description','extra_info']
    const text = fields.map(name => String(form?.elements?.[name]?.value || '')).join(' ')
    return FOOTWEAR_RE.test(text)
  }
  ...
  function cleanGenericSize(raw) {
    ...
    if (/^(?:N\/?A|NOT\s*APPLICABLE)$/i.test(text)) return 'N/A'
    return text
  }

  function parseSize(value, form) {
    const raw = String(value || '').trim()
    if (!raw) return null
    if (SIZE_EXEMPT_RE.test(raw)) return { system: '', size: cleanGenericSize(raw) }
    ...
    if (system) {
      text = text.replace(new RegExp(system, 'ig'), ' ').replace(/[-:]/g, ' ').replace(/\s+/g, ' ').trim()
      if (!text || !/[0-9]/.test(text)) return null
      return { system, size: text }
    }
  ...
  function syncPriceFromVisible(form, validate = false, normalizeVisible = false) {
    ...
    if (!raw) { price.value = ''; currency.value = ''; if (validate) setInvalid(wrapper, false); return true }
  ...
  function syncVisibleFromHidden(form) {
    ...
    if (p && p.dataset.userEdited !== 'true' && document.activeElement !== p) {
      const amount = String(form.elements?.price?.value || '').trim(), currency = String(form.elements?.currency?.value || '').trim()
      if (amount && currency) { const parsed = { currency, amount: Number(amount) }; p.value = canonicalPrice(parsed); p.dataset.canonicalValue = canonicalPrice(parsed) }
    }
  ...
  function enhance() {
    ...
    hideOriginalControl(price); hideOriginalControl(currency)

    const sizeLabel = size.closest('label'), sizeParent = sizeLabel?.parentElement
    ...
    const priceEntry = form.querySelector('[name="price_entry"]'), sizeEntry = form.querySelector('[name="size_entry"]')
    priceEntry?.addEventListener('input', () => { priceEntry.dataset.userEdited = 'true'; syncPriceFromVisible(form, false, false); setInvalid(priceEntry.closest('.inline-unit-field'), false) })
    priceEntry?.addEventListener('blur', () => syncPriceFromVisible(form, true, true))
    sizeEntry?.addEventListener('input', () => { sizeEntry.dataset.userEdited = 'true'; syncSizeFromVisible(form, false, false); setInvalid(sizeEntry.closest('.inline-unit-field'), false) })
  ...
  document.addEventListener('submit', event => {
    ...
    if (!priceOk || !sizeOk) {
      ...
      form.querySelector('.inline-unit-field.invalid')?.scrollIntoView({ behavior: 'smooth', block: 'center' })
    }
  }, true)
