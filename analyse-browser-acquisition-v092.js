(() => {
  ...
  function enrichFromVisual(form, x) {
    ...
    if (form.elements.price && !clean(form.elements.price.value) && Number.isFinite(Number(x.asking_price)) && Number(x.asking_price_confidence) >= 0.90) {
      form.elements.price.value = String(Number(x.asking_price))
    }
    const details = [
  ...
  document.addEventListener('submit', async event => {
    ...
    if (form.dataset.websiteFallbackPass === '1') {
    ...
    try {
      if (files.length) {
        ...
        const useful = Number.isFinite(Number(facts.asking_price)) || (clean(facts.listing_title) && (clean(facts.description) || (Array.isArray(facts.image_urls) && facts.image_urls.length)))
        if (!useful) {
          showScreenshotFallback(form)
          return
        }
      }
