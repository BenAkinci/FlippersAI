(() => {
  ...
  const countBy=(rows,key)=>{const m=new Map();rows.forEach(r=>m.set(r[key],(m.get(r[key])||0)+1));return [...m.entries()].sort((a,b)=>b[1]-a[1]||a[0].localeCompare(b[0]))}
  ...
  function matches(row){return (regionFilter==='ALL'||row.region===regionFilter)&&(categoryFilter==='ALL'||row.category===categoryFilter)}
  function render(){
    const grid=$('.scout-web-grid'),head=$('.scout-web-head')
    if(!grid||!head)return
    const rows=inspect();if(!rows.length)return
    rows.forEach(r=>r.card.hidden=!matches(r))
    ...
    const bullets=[`${rows.length} listings are in this Scout batch for ${query}.`,avg==null?'No reliable asking prices are visible yet.':`Average asking price is ${money(avg)}${min!==max?`, ranging from ${money(min)} to ${money(max)}`:''}.`,regions.length?`Listings span ${regions.map(([x])=>x).join(', ')}.`:'No broad state or territory could be read reliably yet.',cats.length>1?`Mixed products detected across ${cats.map(([x])=>x).join(', ')}. They are being kept separate by category.`:`Results appear to be in the ${cats[0]?.[0]||'Other'} category.`]
    ...
    box.innerHTML=`<div class="smart-web-metrics"><div><span>LISTINGS DETECTED</span><strong>${rows.length}</strong></div><div><span>AVERAGE PRICE</span><strong>${avg==null?'Not available':money(avg)}</strong></div><div><span>AREAS</span><strong>${regions.length?esc(regions.map(([x])=>x).join(', ')):'Not detected'}</strong></div><div><span>CATEGORIES</span><strong>${cats.length===1?esc(cats[0][0]):`${cats.length} detected`}</strong></div></div><div class="smart-web-summary"><span>SCANNED SUMMARY</span><ul>${bullets.map(b=>`<li>${esc(b)}</li>`).join('')}</ul></div>${regions.length?`<div class="smart-web-filter"><span>AREA</span><div>${chip('All','ALL',regionFilter==='ALL','region',rows.length)}${regions.map(([x,n])=>chip(x,x,regionFilter===x,'region',n)).join('')}</div></div>`:''}${cats.length>1?`<div class="smart-web-filter"><span>CATEGORY</span><div>${chip('All','ALL',categoryFilter==='ALL','category',rows.length)}${cats.map(([x,n])=>chip(x,x,categoryFilter===x,'category',n)).join('')}</div></div>`:''}<div class="smart-web-filter-count">${rows.filter(matches).length} listing${rows.filter(matches).length===1?'':'s'} shown</div>`
  }
  document.addEventListener('click',event=>{
    const box=event.target.closest?.('#smartScoutWebOverview');if(!box)return
    const region=event.target.closest?.('[data-region]');if(region){event.preventDefault();regionFilter=region.dataset.region||'ALL';render();return}
    const category=event.target.closest?.('[data-category]');if(category){event.preventDefault();categoryFilter=category.dataset.category||'ALL';render()}
  },true)
  new MutationObserver(mutations=>{
    const meaningful=mutations.some(m=>!m.target.closest?.('#smartScoutWebOverview'))
